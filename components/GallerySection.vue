  ...
  <section id="gallery" class="py-12 px-6 bg-neutral-900/20 border-t border-neutral-900 overflow-hidden">
    <div class="max-w-4xl mx-auto">
      <div class="mb-6 flex items-end justify-between">
        ...
        <div class="flex gap-2" v-if="photosList.length > 1">
          <button 
            @click="prevSlide" 
      ...
      <div v-if="photosList.length" class="relative max-w-2xl mx-auto">
          ...
          <div class="absolute bottom-4 left-4 right-4 z-30 flex justify-between items-end">
            <div>
              <span class="text-[10px] font-bold uppercase tracking-widest text-amber-500 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                {{ currentSlide + 1 }} / {{ photosList.length }}
              </span>
              <p class="text-xs sm:text-sm font-semibold text-neutral-100 mt-1.5">{{ activePhoto.caption }}</p>
            </div>
        ...
        <div class="flex justify-center gap-1.5 mt-3" v-if="photosList.length > 1">
          <button 
            v-for="(_, index) in photosList" 
            :key="index"
    ...
    <!-- Tam Ekran Lightbox -->
    <div v-if="activeLightbox && activePhoto" class="fixed inset-0 z-50 bg-black/95 backdrop-blur-md flex items-center justify-center p-4" @click.self="activeLightbox = false">
      ...
      <div class="max-w-5xl w-full text-center space-y-4">
        <img :src="activePhoto.url" :alt="activePhoto.caption" class="max-h-[80vh] mx-auto rounded-xl object-contain shadow-2xl" />
        <p class="text-sm text-neutral-300 font-medium">{{ activePhoto.caption }}</p>
      </div>
    </div>
  </section>
</template>
...
const photosList = computed(() => props.galleryPhotos || [])

const activePhoto = computed(() => {
  if (photosList.value.length === 0) return { url: '', caption: '' }
  const index = Math.min(currentSlide.value, photosList.value.length - 1)
  return photosList.value[index] || photosList.value[0]
})

// Galeri dizisi değişirse slide indeksini güvenli sınırlar içinde tut
watch(photosList, (newList) => {
  if (currentSlide.value >= newList.length) {
    currentSlide.value = Math.max(0, newList.length - 1)
  }
}, { deep: true })

const nextSlide = () => {
  if (photosList.value.length === 0) return
  currentSlide.value = (currentSlide.value + 1) % photosList.value.length
}

const prevSlide = () => {
  if (photosList.value.length === 0) return
  currentSlide.value = (currentSlide.value - 1 + photosList.value.length) % photosList.value.length
}
</script>